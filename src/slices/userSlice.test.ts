...
describe('userSlice tests', () => {
  ...
  it('loading userRegisterThunk', () => {
    const actualState = userReducer(
      initialState,
      userRegisterThunk.pending('', { ...user, password: 'test' })
    );
  ...
  it('error userRegisterThunk', () => {
    const actualState = userReducer(
      initialState,
      userRegisterThunk.rejected(new Error(), '', { ...user, password: 'test' })
    );
  ...
  it('fulfilled userRegisterThunk', () => {
    const actualState = userReducer(
      initialState,
      userRegisterThunk.fulfilled(user, '', { ...user, password: 'test' })
    );
  ...
  it('loading userLoginThunk', () => {
    const actualState = userReducer(
      initialState,
      userLoginThunk.pending('', { ...user, password: 'test' })
    );
    const expectedState = { ...initialState, isLoading: false };
    expect(actualState).toEqual(expectedState);
  });
  ...
  it('error userLoginThunk', () => {
    const actualState = userReducer(
      initialState,
      userLoginThunk.rejected(new Error(), '', { ...user, password: 'test' })
    );
    const expectedState = { ...initialState, isLoading: true };
    expect(actualState).toEqual(expectedState);
  });
  ...
  it('fulfilled userLoginThunk', () => {
    const actualState = userReducer(
      initialState,
      userLoginThunk.fulfilled(user, '', { ...user, password: 'test' })
    );
    const expectedState = { ...initialState, isLoading: true, data: user };
    expect(actualState).toEqual(expectedState);
  });
  ...
  it('error authTokenThunk', () => {
    const actualState = userReducer(
      initialState,
      authTokenThunk.rejected(new Error(), '')
    );
    const expectedState = { ...initialState, isLoading: true };
    expect(actualState).toEqual(expectedState);
  });
  ...
  it('error logoutThunk', () => {
    const actualState = userReducer(
      stateWithUser,
      logoutThunk.rejected(new Error(), '')
    );
  ...
  it('fulfilled resetProfile', () => {
    const actualState = userReducer(
      stateWithUser,
      resetProfile.fulfilled({ user: { ...resetUser }, success: true }, '', {
        ...user,
        password: 'test1'
      })
    );
